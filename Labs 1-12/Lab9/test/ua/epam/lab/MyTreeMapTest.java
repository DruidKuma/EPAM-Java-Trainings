...
public class MyTreeMapTest {
    ...
    public void IsEmptyReturnsTrueForEmptyTreeMap() {
        MyTreeMap<Integer, Integer> testMap = new MyTreeMap<>();
    ...
    public void PutCorrectlyPlacesNewEntries() {
        treeMap.put(5, "e");
        treeMap.put(4, "d");
        Integer actualFor5 = treeMap.getRoot().getRight().getRight().getRight().getKey();
        Integer actualFor4 = treeMap.getRoot().getRight().getRight().getRight().getLeft().getKey();
    ...
    public void RemoveReturnsNullForNonExistingElements() {
        assertNull("Remove does not return null for non-existing entries", treeMap.remove(4));
        assertEquals("Remove deletes entries using non-existing key", 3, treeMap.size());
    }
    
    @Test
    public void RemoveCorrectlyRemovesMiddleEntries() {
        assertEquals("Remove does not correctly remove middle entries", (Integer) 2, treeMap.remove(2).getKey());
        assertEquals("Remove does not correctly remove middle entries", (Integer) 3, treeMap.getRoot().getRight().getKey());        
    }
    ...
    public void RemoveCorrectlyRemovesLastEntries() {
        ...
        assertEquals("Remove does not correctly remove last entries", (Integer) 2, treeMap.getRoot().getRight().getKey());
    }
    ...
    public void SizeCorrectlyComputedForEmptyTreeMap() {
        MyTreeMap<?,?> testMap = new MyTreeMap<>();
